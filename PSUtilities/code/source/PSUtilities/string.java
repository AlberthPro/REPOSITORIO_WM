...

{
	...
	public static final void StringUtilsContains (IData pipeline)
        throws ServiceException
	{
		// --- <<IS-START(StringUtilsContains)>> ---
		// @sigtype java 3.5
		// [i] field:0:required string
		// [i] field:0:required contains
		// [o] object:0:required result
		String string = IDataUtil.getString(pipeline.getCursor(), "string");
		String contains = IDataUtil.getString(pipeline.getCursor(), "contains");
		
		if (string.toLowerCase().contains(contains.toLowerCase())){
		    IDataUtil.put(pipeline.getCursor(), "result", true);
		}
		else IDataUtil.put(pipeline.getCursor(), "result", false);
		// --- <<IS-END>> ---
