...
%invoke wm.deployer.gui.UIAuditLog:auditLogACLChanges%
	...
	%invoke wm.deployer.UIAuthorization:getLocalGroups%
		%loop groups%			
			<script>addLocalGroup("%value name%", [%loop membership%"%value%"%loopsep ', '% %endloop%]);</script>
		%endloop%
		%include error-handler.dsp%
	%onerror%
		%loop -struct%
...
<form id="form" name="form" method="POST" action="acls.dsp" onsubmit="return submitForm();" >
    ...
    <input type="hidden" name="projectName" value="%value projectName%">
      ...
      <td>
        ...
        <tr><td colspan=3>
        <center>
                    Resulting local users with this Authorization:<br>
