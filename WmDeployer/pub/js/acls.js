...
function buildResultingUsers() {
	...
	for (groupID in AllowGroups)
	{
		...
		for (user in localGroups[groupID])
		{
	...
	{
		for (user in localGroups["Everybody"])
		{
			if (DenyArray[localGroups["Everybody"][user]] == null)
			{
...
function addLocalGroup(group, users) {

	//Arrays must be "prepped" to be containainers of arrays.
	localGroups[group] = ["placeholder"]
	localGroups[group].length = 0
	
	for (user in users)
	{
		localGroups[group][users[user]] = users[user];
	}
