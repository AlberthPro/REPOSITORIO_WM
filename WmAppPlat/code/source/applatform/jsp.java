...

{
		...
		throws ServiceException
	{
		...
		try {
			if ( debug) {
				System.err.println("Calling wm.server:logout for App Platform jsp:cleanupSession service");
				System.err.println("watt.security.session.forceReauthOnExpiration = " +
						Config.getProperty( "watt.security.session.forceReauthOnExpiration"));
			}
			Service.doInvoke("wm.server", "logout", new IDataMap().getIData());
			if ( debug) {
				System.err.println("Session destroyed.");
			}
		} catch (Exception e) {
			if ( debug) {
				System.err.println("Exception calling wm.server:logout IS service for App Platform session");
				e.printStackTrace();
			}
		}
